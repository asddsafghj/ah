...
local token = '471939957:AAFCKYWgFaPdqWJoQgw4nnVUEUBjDZ4s5Yc'  --[[Enter tokn here]]
local url = 'https://api.telegram.org/bot' .. token
local offset = 0
local SUDO = 170146015   --[[Enter your id here]]
local redis = require('redis')
local redis = redis.connect('127.0.0.1', 6379)
function is_mod(chat,user)
	sudo = {170146015}  --[[Enter your id and cli bot id here]]
 local var = false
  ...
  while true do
    ...
    if(updates) then
      if (updates.result) then
		...
		if q.from.id == 366695086 or q.from.id == 170146015 then
 ...
 if redis:get(hash) then
    ...
    return false
              ...
              local keyboard = {}
				 ...
				 {text = 'پشتیبانی ربات زئوس', callback_data = 'supportbot:'..chat},{text = 'تبلیغات شما', callback_data = 'youradds:'..chat}
				  },{
				 {text = 'اطلاعات گروه', callback_data = 'groupinfo:'..chat},{text = 'zeus راهنمای', callback_data = 'helpbot:'..chat}
				}
							}
            answer(q.id,'settings','Group settings',chat,'به بخش اصلی زئوس بات خوش آمدید.\nاز منوی زیر انتخاب کنید:',keyboard)
            end
            end
						end
          if msg.callback_query then
            local q = msg.callback_query
             ...
             if q.data:match('_') and not (q.data:match('next_page') or q.data:match('left_page')) then
                Canswer(q.id,">برای مشاهده راهنمای بیشتر این بخش عبارت\n/help\nرا ارسال کنید\n>تیم پشتیبانی:[@sudo_senator]\n>کانال پشتیبانی:[@Zeusbotsupport]\n>کانال فروش:[@sudo_senator]",true)
...
local hash = SUDO..'settings:'..chat..':'..value
 if redis:get(hash) then
    ...
    return false
            ...
            edit(q.inline_message_id,'`به بخش اصلی zeusbot خوش آمدید.`\n`از منوی زیر انتخاب کنید:`',keyboard)
            end
							------------------------------------------------------------------------
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`جهت ثبت و درخواست تبلیغات به سازنده ربات به آیدی @sudo_senator مراجعه کنید.`',keyboard)
            end
							------------------------------------------------------------------------
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'>[فیلم های آموزشی ما در آپارات](https://aparat.com/)\n\n*فیلم های آموزشی ما را در کانال آپارات دنبال کنید*\nhttps://aparat.com/',keyboard)
            end
							------------------------------------------------------------------------
